    ...
    for file in files:
        if file and file.filename.endswith('.pdf'):
            try:
                ...
                all_files_results.append({"filename": file.filename, "error": str(e)})

        else:
            all_files_results.append({"filename": file.filename, "error": "Invalid file format. Only PDF files are supported."})
    
    return jsonify(all_files_results), 200
